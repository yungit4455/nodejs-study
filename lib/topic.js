    ...
    db.query(`SELECT * FROM topic`, (error, topics) => {
        ...
        [req.params.topicsId], (error2, topic) => {
            ...
            const html = template.HTML(sanitizeHtml(title), list, 
                ...
                `,
                `<a href="/topics">create</a> 
                 <a href="/update?id=${req.params.topicsId}">update</a>
...
exports.create_process = (request, response) => {
    ...
    request.on('end', () => {
        ...
        VALUES(?, ?, NOW(), ?)`,
        [sanitizeHtml(post.get('title')), sanitizeHtml(post.get('description')), sanitizeHtml(post.get('author'))],
        (error, result) => {
            if(error) { throw error; }
        ...
        db.query(`SELECT * FROM topic WHERE id=?`, [queryData.id], (error2, topic) => {
            ...
            db.query(`SELECT * FROM author`, (error3, authors) => {
                ...
                const html = template.HTML(sanitizeHtml(topic[0].title), list, 
                    ...
                    `,
                    `<a href="/topics">create</a> <a href="/update?id=${topic[0].id}">update</a>`
                );
                response.writeHead(200);
                response.end(html);  
            });
...
exports.update_process = (request, response) => {
    ...
    request.on('end', function() {
        const post = new URLSearchParams(body);
        db.query(`UPDATE topic SET title=?, description=?, author_id=? WHERE id=?`, [sanitizeHtml(post.get('title')), sanitizeHtml(post.get('description')), sanitizeHtml(post.get('author')), post.get('id')], 
        (error, result) => {
            if(error) { throw error; }
